...
self.onmessage = function (_ref) {
  ...
  var _loop = function _loop(coord) {
    // Four variable names pointing to the same coordinate: A1, a1, $A1, $a1
    ["", "$"].map(function (p) {
      return [coord, coord.toLowerCase()].map(function (c) {
        var name = p + c; // Worker is reused across computations, so only define each variable once
        ...
        Object.defineProperty(self, name, {
          get: function get() {
            ...
            vals[coord] = NaN; // Turn numeric strings into numbers, so =A1+C1 works when both are numbers
